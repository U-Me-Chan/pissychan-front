const ru = {
  reply: 'Ответить',
  up: 'Вернуться в доску',
  name: 'Имя',
  subject: 'Тема',
  message: 'Суть (10000 символов максимум, остальное будет обрезано)',
  submit: 'Отправить',
  pissychan: 'Писсичан',
  posting_mode: 'Режим постинга',
  posting_mode_reply: 'Ответ в тред',
  posting_mode_forbidden: 'Постинг запрещён',
  parent_thread: 'Родительский тред',
  into_thread: 'В тред',
  post_count: 'Ответов в треде',
  feed: 'Недавнее',
  link_home: 'Глагне',
  sage: 'сажа',
  months: ['Янв', 'Фев', 'Мар', 'Апр', 'Май', 'Июн', 'Июл', 'Авг', 'Сен',
    'Окт', 'Ноя', 'Дек']
}

const en = {
  reply: 'Reply',
  up: 'Return',
  name: 'Name',
  subject: 'Subject',
  message: 'Message (10000 symbols max, exceeding text will be truncated)',
  submit: 'Submit',
  pissychan: 'Pissychan',
  posting_mode: 'Posting mode',
  posting_mode_reply: 'Reply',
  posting_mode_forbidden: 'Forbidden',
  parent_thread: 'Parent thread',
  into_thread: 'Reply',
  post_count: 'Posts count',
  feed: 'Feed',
  link_home: 'Home',
  sage: 'sage',
  months: ['Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep',
    'Oct', 'Nov', 'Dec']
}

module.exports = {
  ru,
  en
}
